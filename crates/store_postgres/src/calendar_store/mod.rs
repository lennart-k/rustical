...
impl PostgresCalendarStore {
    ...
    pub async fn validate_objects(&self, principal: &str) -> Result<(), Error> {
        ...
        if !success {
            if self.skip_broken {
                error!(
                    "Not all calendar objects are valid. Since data_store.postgres.skip_broken=true they will be hidden. You are still advised to manually remove or repair the object. If you need help feel free to open up an issue on GitHub."
                );
            } else {
                error!(
    ...
    pub async fn repair_invalid_version_4_0(&self) -> Result<(), Error> {
        ...
        for repair in &repairs {
            ...
            sqlx::query!(
                "DELETE FROM calendarobjectchangelog WHERE (principal, cal_id, object_id) = ($1, $2, $3);",
                repair.principal, repair.cal_id, repair.id
            ).execute(&mut *tx).await
            .map_err(crate::Error::from)?;

            sqlx::query!(
                "UPDATE calendarobjects SET ics = $1 WHERE (principal, cal_id, id) = ($2, $3, $4);",
                repair.ics,
                repair.principal,
                repair.cal_id,
                repair.id
    ...
    pub async fn repair_orphans(&self) -> Result<(), Error> {
        ...
        let rows = sqlx::query_as!(
            Row,
            r#"
                SELECT principal, cal_id, id, (deleted_at IS NOT NULL) AS "deleted!"
    ...
    async fn _get_calendar<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        id: &str,
        show_deleted: bool,
    ...
    async fn _get_calendars<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        ...
        let cals = sqlx::query_as!(
            ...
            r#"SELECT *
                FROM calendars
                WHERE principal = $1 AND deleted_at IS NULL"#,
            principal
        )
        .fetch_all(executor)
        .await
    ...
    async fn _get_deleted_calendars<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
    ) -> Result<Vec<Calendar>, Error> {
        let cals = sqlx::query_as!(
            ...
            r#"SELECT principal, id, displayname, "order", description, color, timezone_id, deleted_at, synctoken, subscription_url, push_topic, comp_event, comp_todo, comp_journal
                FROM calendars
                WHERE principal = $1 AND deleted_at IS NOT NULL"#,
            principal
        )
        .fetch_all(executor)
        .await
    ...
    async fn _insert_calendar<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        calendar: Calendar,
    ) -> Result<(), Error> {
    ...
    async fn _update_calendar<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        id: &str,
        calendar: Calendar,
        ...
        let result = sqlx::query!(
            r#"UPDATE calendars SET principal = $1, id = $2, displayname = $3, description = $4, "order" = $5, color = $6, timezone_id = $7, push_topic = $8, comp_event = $9, comp_todo = $10, comp_journal = $11
                WHERE (principal, id) = ($12, $13)"#,
            calendar.principal,
            calendar.id,
            calendar.meta.displayname,
            calendar.meta.description,
    ...
    async fn _delete_calendar<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        id: &str,
        use_trashbin: bool,
        ...
        if use_trashbin {
            sqlx::query!(
                r#"UPDATE calendars SET deleted_at = CURRENT_TIMESTAMP WHERE (principal, id) = ($1, $2)"#,
                principal,
                id
            )
            .execute(executor)
            ...
            sqlx::query!(
                r#"DELETE FROM calendars WHERE (principal, id) = ($1, $2)"#,
                principal,
                id
            )
            .execute(executor)
    ...
    async fn _restore_calendar<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        id: &str,
    ) -> Result<(), Error> {
        sqlx::query!(
            r"UPDATE calendars SET deleted_at = NULL WHERE (principal, id) = ($1, $2)",
            principal,
            id
        )
        .execute(executor)
    ...
    async fn _prune_deleted_calendars<'e, E: Executor<'e, Database = Postgres>>(
        ...
        sqlx::query!(
            r"DELETE FROM calendars WHERE deleted_at IS NOT NULL AND deleted_at::date < $1::date",
            before,
        )
        .execute(executor)
        .await
    ...
    async fn _list_objects<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        cal_id: &str,
        ...
        Ok(sqlx::query_as!(
            ObjectEntry,
            r#"SELECT octet_length(ics) AS "length!", (deleted_at IS NOT NULL) AS "deleted!" FROM calendarobjects WHERE principal = $1 AND cal_id = $2"#,
            principal,
            cal_id
        )
        .fetch_all(executor)
        .await.map_err(crate::Error::from)?
        .into_iter()
        .map(|row| (row.length.cast_unsigned(), row.deleted))
        .collect())
    }

    async fn _get_objects<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        cal_id: &str,
        ...
        Ok(sqlx::query_as!(
            CalendarObjectRow,
            "SELECT id, uid, ics FROM calendarobjects WHERE principal = $1 AND cal_id = $2 AND deleted_at IS NULL",
            principal,
            cal_id
        )
        .fetch_all(executor)
    ...
    async fn _calendar_query<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        cal_id: &str,
        query: CalendarQuery,
    ...
    async fn _get_object<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        cal_id: &str,
        object_id: &str,
        ...
        let (row_id, object) = sqlx::query_as!(
            CalendarObjectRow,
            "SELECT id, uid, ics FROM calendarobjects WHERE (principal, cal_id, id) = ($1, $2, $3) AND ((deleted_at IS NULL) OR $4)",
            principal,
            cal_id,
            object_id,
            show_deleted
    ...
    async fn _put_object(
        ...
        if overwrite {
            ...
            sqlx::query!(
                "INSERT INTO calendarobjects (principal, cal_id, id, uid, ics, first_occurence, last_occurence, etag, object_type) VALUES ($1, $2, $3, $4, $5, $6::date, $7::date, $8, $9)",
                principal,
                cal_id,
                object_id,
                uid,
    ...
    #[instrument]
    async fn _delete_object<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        cal_id: &str,
        object_id: &str,
        ...
        if use_trashbin {
            sqlx::query!(
                "UPDATE calendarobjects SET deleted_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE (principal, cal_id, id) = ($1, $2, $3)",
                principal,
                cal_id,
                object_id
            )
    ...
    #[instrument]
    async fn _restore_object<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        principal: &str,
        cal_id: &str,
        object_id: &str,
        ...
        sqlx::query!(
            r#"UPDATE calendarobjects SET deleted_at = NULL, updated_at = CURRENT_TIMESTAMP WHERE (principal, cal_id, id) = ($1, $2, $3)"#,
            principal,
            cal_id,
            object_id
        )
    ...
    async fn _sync_changes<'a, A: Acquire<'a, Database = Postgres>>(
        acquire: A,
        principal: &str,
        cal_id: &str,
        synctoken: i64,
    ...
    async fn _prune_deleted_objects<'e, E: Executor<'e, Database = Postgres>>(
        executor: E,
        before: chrono::NaiveDate,
    ) -> Result<u64, Error> {
        sqlx::query!(
            r"DELETE FROM calendarobjects WHERE deleted_at IS NOT NULL AND deleted_at::date < $1::date",
            before,
        )
        .execute(executor)
        .await
...
#[async_trait]
impl CalendarReadStore for PostgresCalendarStore {
    #[instrument]
...
#[async_trait]
impl CalendarWriteStore for PostgresCalendarStore {
    #[instrument]
...
#[async_trait]
impl CalendarStorePruneDeleted for PostgresCalendarStore {
